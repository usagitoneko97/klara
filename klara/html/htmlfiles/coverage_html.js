...
coverage.pyfile_ready = function ($) {
    ...
    $(".t.float_warning").click(function (evt) {coverage.toggle_lines(evt.target, "mis");})
    $(".infer-btn-clear").click(function (evt) {
        coverage.clear_all_paths()
        $(".popover").popover("hide")
        coverage.end_infer_highlight()
        coverage.current_id_value = -1
        coverage.current_ids = null
    })
    $(".infer-btn-next").click(function (evt) {
        coverage.next_infer_highlight()
    })
    $(".infer-btn-prev").click(function (evt) {
        coverage.next_infer_highlight(false)
    })

    $("#next_violating").click(function (evt) {
        coverage.to_next_chunk_nicely()
    })

    $("#prev_violating").click(function (evt) {
        coverage.to_prev_chunk_nicely()
    })

    $(".float_warning").click(function (evt) {
        let target = $(evt.target)
        let infer_paths = target.attr("infer_paths")
        // let json_obj = JSON.parse(infer_paths)
        // coverage.toggle_warning_paths(json_obj)
    })

    $(".nam").click(coverage.infer_node)
...
coverage.infer_node = function (evt) {
    let target = $(evt.target)
    coverage.end_infer_highlight()
    $.getJSON("infer", {
        lineno: target.attr("lineno"),
        col: target.attr("col")
    }, function (data) {
        if (parseInt(data["status"]) === 500) {
            console.warn(`receive status 500 for node at lineno: ${target.attr("lineno")}, 
                col: ${target.attr("col")}`)
            return
        }
        coverage.toggle_warning_paths(data["inference_result"], target)
    })
}

coverage.highlight_paths = function (paths) {
    $.each(paths, function (_, infer_result_path) {
        $.each(infer_result_path["ids"], function (_, id) {
            console.log(`changing id: ${id} to infer path!`)
            let warning = $(`#${id}`)
            if (!warning.hasClass("infer_path")) {
                warning.addClass("infer_path")
            }
        })
    })
}

coverage.clear_all_paths = function () {
    $(".infer_path").each(function (_) {
        $(this).removeClass("infer_path")
    })
}

coverage.toggle_warning_paths = function (json_obj, target) {
    coverage.clear_all_paths()
    $.each(json_obj, function (_, value) {
        coverage.highlight_paths(value["path"])
    })
    let result_html = ""
    let button_count = 0
    let button_map = {}
    $.each(json_obj, function (_, json_value) {
        if (Object.keys(json_value["path"]).length > 0) {
            let id = `b${button_count}`
            result_html += `<span id="${id}" class="infer_result p-2 bd-highlight badge badge-dark">${json_value["result"]}</span>`
            result_html += `<span>${json_value["result_type"]}</span>\n`
            result_html += "<hr>"
            result_html += `<span>${json_value["bound_conditions"]}</span>\n`
            result_html += "<hr>"
            button_map[id] = json_value["path"]
            button_count += 1
        } else {
            console.warn("Getting no path")
        }
    })
    if (button_count > 0) {
        result_html = "<h6>Inference result</h6>" +  `<div class='d-flex flex-column justify-content-around bd-highlight mb-${button_count}'>` + result_html
        result_html += "</div>"
        target.popover({content: result_html, trigger: "focus", html: true})
        target.popover("toggle")
        $(".infer_result").click(function (evt) {
            let infer_result_target = $(evt.target)
            let paths = button_map[infer_result_target.attr("id")]
            coverage.clear_all_paths()
            coverage.end_infer_highlight()
            coverage.highlight_paths(paths)
            target.popover("hide")
            coverage.start_infer_highlight(paths, target)
            // call next twice to skip the current highlighted node
            coverage.next_infer_highlight()
            coverage.next_infer_highlight()
        })
    }
...
coverage.start_infer_highlight = function(paths, target) {
    var sorted = Object.keys(paths) //Create a list from the keys of your map.
        .sort( //Sort it ...
    ...
    sorted.forEach(function (value) {
        ids.push(paths[value]["ids"])
    })
    coverage.current_ids = ids
    coverage.current_id_value = -1
    $(".infer-btn-next").collapse("show")
    $(".infer-btn-prev").collapse("show")
}

coverage.end_infer_highlight = function() {
    $(".borderBlink").removeClass("borderBlink")
    $(".infer-btn-next").collapse("hide")
    $(".infer-btn-prev").collapse("hide")
}

coverage.next_infer_highlight = function(next_direction=true) {
    coverage.current_id_value = next_direction? coverage.current_id_value + 1:coverage.current_id_value - 1
    prev_id_value = next_direction? coverage.current_id_value - 1:coverage.current_id_value + 1
    if (prev_id_value >= 0) {
        // remove border blink for prev element
        coverage.current_ids[prev_id_value].forEach(function (item, index, array) {
            let id_elem = $(`#${item}`)
            id_elem.removeClass("borderBlink")
        })
    }
    if (coverage.current_id_value >= coverage.current_ids.length || coverage.current_id_value < 0) {
        console.warn(`requested id: ${coverage.current_id_value} exceed ids length:${coverage.current_ids.length}`)
        $("#exampleModal").modal("show")
        return
    }
    coverage.current_ids[coverage.current_id_value].forEach(function (item, index, array) {
        let id_elem = $(`#${item}`)
        id_elem.addClass("borderBlink")
    })
    let id = coverage.current_ids[coverage.current_id_value][0]
    let id_elem = $(`#${id}`)
    if (! id_elem.isOnScreen()) {
        let top_pos = parseInt(id_elem.offset().top, 10);
        coverage.scroll_window(top_pos - 200);
    }
}
...
coverage.to_next_infer = function () {
    var c = coverage;

    // Find the start of the next colored chunk.
    var probe = c.infer_sel_end;
